...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            ...
            zp_private_selftest (verbose, item->subtest);
    }
...
main (int argc, char **argv)
{
    ...
    if (test) {
        printf ("Running zp test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
        else
            zp_private_selftest (verbose, test->subtest);
    }
    else
        test_runall (verbose);
